...
internal sealed class EfWorkflowStore(WorkflowDbContext dbContext) : IWorkflowStore
{
    ...
    public async Task<ApplicationResult<ProvisioningOperation>>
    ...
    private static ApplicationResult<T> ReadUnavailable<T>()
        where T : notnull =>
        ApplicationResult.Failed<T>(WorkflowPersistenceFailures.Unavailable());
}
