...
adds structured `environmentOptionIds` so the application can validate and render
authoritative alternatives instead of trusting free-form model text.
...
| AI and MCP boundary | PASS | The design uses exactly `get_production_environment` and `get_incident`, keeps SDK contracts in adapters, schema-validates output, and independently reloads environment-role evidence. |
...
| AI and MCP boundary | PASS | The contract advertises exactly two closed-schema tools. Environment discovery returns bounded authoritative context; fallback suggestions are catalog members and remain untrusted; incident lookup remains exact-only. |
...
`-- GovernedAccess.Web/
    ...
    |-- Teams/TeamsAccessRequestAgent.cs         # authoritative option rendering
...
3. Update model instructions, proposal-schema validation, and catalog validation.
   ...
   the referenced contexts, sorts them, and renders their authoritative display
   values. Incident text never triggers discovery, role choices come only from the
   selected environment, and `clientId` is never a clarification target.
4. Update deterministic chat substitutes and contract, boundary, session, persistence,
   and Teams intake tests. Cover exact `NotFound` followed by discovery, one/multiple/
   zero plausible alternatives, explicit confirmation, conflicts, and the absence of
   fallback for every other failure. Preserve all existing `RequestValidator` role
   checks and workflow regression coverage.
