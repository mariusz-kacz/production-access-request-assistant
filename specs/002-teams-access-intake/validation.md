...
| 3. Discuss and revise immutable draft | `RequestDraftAndSubmissionServiceTests.DraftDiscussionPreservesReadyCandidateAndConfirmationIdentity`, `RequestDraftAndSubmissionServiceTests.NaturalLanguageEditCarriesReadyCandidateIntoReplacementPreparation`, and `TeamsRequestPreparationTests.NaturalLanguageEditRevisesReadyDraftFromValidatedCandidate` | PASS - discussion preserves the active draft; a changed validated candidate supersedes it, displays a replacement, and creates no request. T096 tracks direct `UpdateActivityAsync` presentation verification. |
