...
| 3. Discuss and revise immutable draft | `RequestDraftAndSubmissionServiceTests.DraftDiscussionPreservesReadyCandidateAndConfirmationIdentity`, `RequestDraftAndSubmissionServiceTests.RevisionClarificationPreservesReadyCandidateAndConfirmationIdentity`, `RequestDraftAndSubmissionServiceTests.NaturalLanguageEditCarriesReadyCandidateIntoReplacementPreparation`, `TeamsRequestPreparationTests.EnvironmentRevisionClarificationKeepsExistingDraftCardActiveUntilReplacementIsReady`, and `TeamsRequestPreparationTests.NaturalLanguageEditRevisesReadyDraftFromValidatedCandidate` | PASS - discussion and pending revision clarification preserve the active draft; a different ready candidate supersedes it, displays a replacement, and creates no request. T096 tracks direct `UpdateActivityAsync` presentation verification. |
