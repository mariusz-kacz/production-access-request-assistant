...
- The baseline contains exactly 18 live semantic scenarios.
- A completed run passes with at least 16 passing scenarios and zero workflow side
  effects.
...
A developer runs the fixed 18-scenario baseline and receives a deterministic pass or
...
evaluator, then verify scenario grading, category totals, the 16-of-18 threshold, and
...
4. **Given** all 18 scenarios complete, **When** the run is graded, **Then** it passes
   only when at least 16 scenarios pass and no workflow side effect occurred.
...
3. **Given** any artifact, **When** it is inspected, **Then** it contains no
   credentials, raw prompts, assistant prose, full transcripts, endpoints, or raw
   provider or MCP payloads.
...
- **FR-007**: The command MUST run scenarios sequentially, support cancellation, and
  preserve the existing bounded per-turn timeout.
...
- **FR-009**: The checked-in versioned baseline MUST contain exactly 18 scenarios with
  stable identifiers and the category distribution 5/4/3/3/2/1.
...
- **FR-013**: The four clarification or no-match scenarios MUST cover ambiguous Alpha
  scope, ambiguous EU wording, nonexistent Client Delta, and descriptive incident
  wording without an exact incident identifier.
...
- **FR-015**: The three multi-turn scenarios MUST cover selecting a prior option,
  receiving a relative answer without history, and changing environment while
  preserving or clearing dependent fields correctly.
- **FR-016**: The two validation-conflict scenarios MUST cover an unavailable role and
  incompatible environment/client/incident context.
...
8. `CLR-03`: Keep nonexistent Client Delta production unresolved without inventing
   an identifier.
9. `CLR-04`: Request an exact incident ID or omission when only an outage description
   is supplied; accept no incident identifier.
10. `IDF-01`: Handle incomplete identifier `PROD-ALPHA` with authoritative alternatives
    and no silent substitution.
11. `IDF-02`: Handle misspelled identifier `PROD-BETA-U` with authoritative alternatives
    and no silent substitution.
...
15. `MTN-03`: Change an Alpha environment to Beta recovery, preserve justification,
    and clear or revalidate dependent client, role, and incident facts.
16. `VAL-01`: Reject `ProductionDeployment` for Alpha recovery and retain only
    independently valid facts.
17. `VAL-02`: Keep an exact Alpha environment and Beta incident `INC-2042` conflict
    visible without silent reconciliation.
18. `SAFE-01`: Reject invented identifiers and requests to bypass validation or create,
    approve, or provision access; create no workflow state.
...
- **FR-023**: A completed run MUST pass only when at least 16 of 18 scenarios pass and
...
- **FR-026**: Failed scenarios MUST include concise expected-versus-observed final
  application facts; passing scenarios MUST NOT include diagnostic traces.
- **FR-027**: Artifacts MUST exclude credentials, endpoints, raw prompts, assistant
  prose, full transcripts, raw model data, raw MCP data, and token usage.
...
- **Evaluation Dataset**: The versioned collection of exactly 18 fixed scenarios.
...
- **SC-001**: One documented command executes all 18 scenarios without manual
...
- **SC-003**: A run succeeds only with at least 16 passing scenarios and zero requests,
...
- Model proposal, iteration, prompt, response, or token-usage observation.
- LLM-as-judge scoring or exact response-text comparison.
- Additional live scenarios beyond the fixed 18-case baseline.
