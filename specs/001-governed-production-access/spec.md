...
As an authenticated requester, I describe why I need temporary access to a client production environment, review a structured draft prepared with authoritative context, correct any missing or invalid fields, and submit a valid request for approval.
...
**Independent Test**: Create a valid Client Alpha request, attempt approval as both Client Beta and Client Alpha business approvers, and verify that only the authoritative Client Alpha approver can change the request state.
...
3. **Given** a current request awaiting business approval, **When** the authoritative business approver rejects it, **Then** the request moves to `Rejected` and the authenticated decision is recorded.
...
**Independent Test**: Start with a current business-approved request, approve it as DevOps, and verify that authoritative state is reloaded, the exact approved scope is enforced, and one synthetic time-limited grant becomes active.
...
1. **Given** a request in `ProvisioningFailed`, **When** the authenticated DevOps approver invokes retry without changing approved scope, **Then** the system repeats complete authoritative revalidation using the same operation identity and records the retry outcome.
...
- **FR-011**: The system MUST resolve the required business approver from current authoritative configuration for the target environment; the requester MUST NOT select or supply that approver.
...
- **FR-026**: Retry MUST invoke the same full authoritative revalidation and idempotent provisioning behavior as the initial attempt.
...
- **FR-036**: Operational evidence MUST allow request preparation, authoritative context lookups, authorization decisions, workflow transitions, and provisioning attempts to be correlated and MUST capture duration and outcome where an external or model-assisted operation occurs.
...
- **SC-002**: For 100% of acceptance tests, malformed or incomplete model output and invalid authoritative values result in correction guidance or a typed failure and never create a submitted approval or access grant.
...
- Expected failures are presented in understandable, distinguishable forms, and all time-sensitive assisted or authoritative operations have finite bounds chosen during planning.
- The product baseline at `docs/governed-production-access-product-baseline.md` is the authoritative dependency for fixed demonstration data, security boundaries, and scope.

### Scope Boundaries

- Included: model-assisted typed drafting, exactly three read-only authoritative context operations, deterministic validation, two authenticated approval stages, version-bound decisions, immediate independently validated synthetic provisioning, safe retry, logical expiry, audit evidence, and a three-page React user experience served by the single host.
...
- Stable synthetic authoritative records must exist for the two clients, environments, supported roles, incidents, approver assignments, and four demonstration principals.
...
- Current authoritative state must be available at both submission and provisioning time so stale-state rules can be verified.
