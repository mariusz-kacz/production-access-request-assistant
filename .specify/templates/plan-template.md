...
- **Provisioning evidence**: Keep provisioning model-inaccessible, reload persisted
  evidence in the handler, reject caller assertions, and use request ID for
  idempotency.
- **Proportionality**: Retain the single executable modular host and justify every new
  project, abstraction, process, or infrastructure component.
- **Verification and operations**: Plan applicable unit, integration, and negative
  tests; deterministic AI tests; cancellation/timeouts; typed failures; safe logging;
  and audit evidence.

Record PASS or a concrete violation for every gate. A violation requires an entry in
Complexity Tracking and an approved constitution amendment before implementation.
