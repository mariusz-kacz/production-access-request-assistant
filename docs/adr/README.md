...
| [0012: Deterministic routing and isolated specialist contexts](0012-router-and-context-isolation.md) | Accepted | Validate one closed model route and dispatch through application code to zero or one specialist with separately constructed context and capabilities. | The target can add policy detours while preserving Access Request's canonical-state boundary; bounded route history remains non-authoritative and outside Access interpretation. |
| [0013: Authoritative policy facts and bounded hybrid grounding](0013-policy-grounding.md) | Accepted | Share one typed policy snapshot, retrieve current policy through bounded server-owned Azure hybrid search, and keep the Policy Advisor read-only with a finite runtime contradiction guard. | Current citations and represented facts receive deterministic checks; broader free-form semantic consistency remains an explicit promotion-evaluation risk. |
| [0014: Separate routed evaluation and observability](0014-routed-evaluation-and-observability.md) | Accepted | Use Microsoft evaluation/reporting plus exact product gates and measure router, retrieval, specialist, and end-to-end behavior separately without raw content. | Pre-results thresholds and 100% safety/isolation gates govern promotion while component cost and failure remain diagnosable. |
