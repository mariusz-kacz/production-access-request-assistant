...
| [0007: Sparse model proposals and deterministic reducer](0007-use-sparse-model-patches-and-a-deterministic-reducer.md) | Accepted | Every non-`/new` free-text turn is agent-interpreted into a closed sparse proposal; Core owns structural validation, one atomic scope group, independent justification, authority, lifecycle, and outcomes. | Omission cannot erase state, Core does not become a second NLP parser, and dependent scope does not partially apply. |
...
| [0012: Deterministic routing and isolated specialist contexts](0012-router-and-context-isolation.md) | Accepted; history details partially superseded by 0015 | Validate one closed model route and dispatch through application code to zero or one specialist with separately constructed context and capabilities. | The target can add policy detours while preserving Access Request's canonical-state boundary; bounded route history remains non-authoritative and outside Access interpretation. |
| [0013: Authoritative policy facts and bounded hybrid grounding](0013-policy-grounding.md) | Accepted; prose validation/indexing partially superseded by 0015 | Share one typed policy snapshot, retrieve current policy through bounded server-owned Azure hybrid search, and keep Policy Advisor read-only. | Structural/citation/rendering checks and snapshot precedence remain; runtime validation does not prove prose semantics, and offline evaluation cannot guarantee each live answer. |
| [0014: Separate routed evaluation and observability](0014-routed-evaluation-and-observability.md) | Accepted; router metrics partially superseded by 0015 | Use Microsoft evaluation/reporting plus exact product gates and measure router, retrieval, specialist, and end-to-end behavior separately without raw content. | Pre-results thresholds and 100% safety/isolation gates govern promotion while component cost and failure remain diagnosable. |
| [0015: Refine router/policy target contracts](0015-refine-router-policy-target-contracts.md) | Accepted target amendment | Remove mandatory prose parsing and router judging; specify persisted whole-pair history and fixture-only rebuilds with exact current chunk IDs. | Direct exact router checks use Microsoft reporting; policy quality evaluators remain; pair ordering/overflow and stale-ID removal have focused future acceptance cases. No runtime implementation or promotion. |
