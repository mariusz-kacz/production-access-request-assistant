...
public sealed class RequestQueriesTests
{
    ...
    public async Task ListsAreParticipantFilteredAndMarkOnlyCurrentActions()
    {
        ...
        Assert.DoesNotContain(
            betaItems,
            item => item.RequestId == alphaAwaitingBusiness
                || item.RequestId == alphaAwaitingDevOps);
        ...
        var filteredRequesterItems = await GetListAsync(
            ...
            cancellationToken);
        Assert.Equal(
            new[] { alphaAwaitingBusiness, betaAwaitingBusiness }.Order(),
            filteredRequesterItems.Select(item => item.RequestId).Order());
